...
function addFlavor(arr2, flavor2){

    arr2.unshift(flavor2);
}
...
function getFlavorByIndex(arr4, index4){
    console.log(arr4[index4]);
}
...
as opposed to just arbitrarily removing the first or last flavor. Your task is to get an index by flavor name, and remove that 
...
*/
function removeFlavorByName(arr5, flavor5){
    
    let removed = arr5.indexOf(flavor5);
    arr5.splice(removed, 1);
}
...
chocolate flavors. Write a function that checks every item in the array for a given string and returns a new array called filteredArray 
with just these values. Rather than hardcoding "chocolate" into your function, pass a string as a parameter, and invoke 
...
function filterByWord(arr7, str7){

    let newArray = [];

    for(let i = 0; i < arr7.length; i++){
        if(arr7[i].includes(str7)){

            newArray.push(arr7[i])
        }
    }
    return newArray;
